...
class kriptografi{	
	...
	function EnkripCaesar($data, $kunci){
		...
		for($i=0; $i < strlen($data); $i++){
			$array_pl[$i] =32+((ord($data[$i])+strlen($kunci))%95);
		}
		return $array_pl;
	}
	...
	function HasilEnkripsi($cipherteks){
		...
		for($i=0; $i< count($cipherteks); $i++){
			...
			do{
				...
				$cipherteks[$i] = intdiv($cipherteks[$i], 90);
				if($id==2){
					$cipherteks[$i]=0;
				}

			}while ($cipherteks[$i] > 0);
	...
	function HasilDeskripsi($result,$key){
		...
		for($i=0; $i<$ukur;$i++){
			$result[$i] -= 32;
			if(($result[$i]-strlen($key)) < 32){
				$x = 95 + (($result[$i]-strlen($key)) % 95);	
			}else{
				$x = (($result[$i]-strlen($key)) % 95);
			}
